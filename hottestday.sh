...
if [[ ${choice} == "y" ]]; then
	...
	touch hottestday_HQ_$city.txt
	while [[ "${Date}" < "${final_date}" ]]
	do
		next_date=$(date +%Y-%m-%d -d "$Date +$i year")
		stop_date=$(date +%Y-%m-%d -d "$next_date -1 day")
		awk -v start="${Date}" -v stop="${stop_date}" 'BEGIN{a=-100; b=substr($1,1,4)} $1 == start,$1 == stop {if ($2>a) {a=$2; b=substr($1,1,4)} fi} END{print b" "a}' oneDayTemp_highQuality_$city.txt >> hottestday_HQ_$city.txt
		Date=${next_date}
	done
else
	if [[ -f hottestday_$city.txt ]]; then
		rm hottestday_$city.txt
	fi
	...
	touch hottestday_$city.txt
	while [[ "${Date}" < "${final_date}" ]]
	do
		next_date=$(date +%Y-%m-%d -d "$Date +$i year")
		stop_date=$(date +%Y-%m-%d -d "$next_date -1 day")
		awk -v start="${Date}" -v stop="${stop_date}" 'BEGIN{a=-100; b=substr($1,1,4)} $1 == start,$1 == stop {if ($2>a) {a=$2; b=substr($1,1,4)} fi} END{print b" "a}' oneDayTemp_allEntries_$city.txt >> hottestday_$city.txt
		Date=${next_date}
	done	
fi
